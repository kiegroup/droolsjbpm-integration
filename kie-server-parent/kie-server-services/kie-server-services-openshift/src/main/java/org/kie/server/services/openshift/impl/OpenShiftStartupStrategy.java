...
public class OpenShiftStartupStrategy implements StartupStrategy {
    ...
    public void startup(KieServerImpl kieServer, ContainerManager containerManager, KieServerState currentState, AtomicBoolean kieServerActive) {
        ...
        // Register with controller (Business Central) to enable KieServer visibility in UI
        ...
        try {
            ...
            logger.info("Successfully registered KieServer with controller");
        } catch (KieControllerNotDefinedException e) {
            logger.debug("No controller defined, skipping controller registration");
            // no-op - controller not configured
        } catch (KieControllerNotConnectedException e) {
            logger.warn("Unable to connect to controller, will retry in background thread");
            // Start background thread to retry controller connection
