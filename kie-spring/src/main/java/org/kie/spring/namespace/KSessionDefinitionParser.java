...
import static org.kie.spring.factorybeans.helper.StatefulKSessionFactoryBeanHelper.JpaConfiguration;
...
public class KSessionDefinitionParser extends AbstractBeanDefinitionParser {
    ...
    private static final String KSESSION_ELEMENT_NAME = "ksession";
    ...
    private static final String RELEASE_ID_ATTRIBUTE = "releaseId";
    ...
    protected AbstractBeanDefinition parseInternal(Element element,
        ...
        String kbaseRef = element.getAttribute("kbase-ref");
        if (KSESSION_ELEMENT_NAME.equalsIgnoreCase(element.getLocalName())) {
            emptyAttributeCheck(element.getLocalName(), "kbaseName", kbaseRef);
        }
        if (StringUtils.hasLength(kbaseRef)) {
            factory.addPropertyValue("kbaseName", kbaseRef);
        }

        factory.addPropertyValue(ID_ATTRIBUTE, id);
        factory.addPropertyValue(TYPE_ATTRIBUTE, type);

        String releaseIdRef = element.getAttribute(RELEASE_ID_ATTRIBUTE);
        if (StringUtils.hasLength(releaseIdRef)) {
            factory.addPropertyReference(RELEASE_ID_ATTRIBUTE, releaseIdRef);
        }
